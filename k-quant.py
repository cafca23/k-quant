                ...
                with st.spinner(f"[{company_name}]의 수급 데이터와 4차원 매트릭스를 분석하여 AI 브리핑을 작성 중입니다... 🧠"):
                    try:
                      ...
                      prompt = f"""
                        ...
                        - 펀더멘털: ROE {roe*100:.1f}% / 배당성향 {payout_ratio*100:.1f}% / 공매도 {short_text if 'short_text' in locals() else 'N/A'}
                        - 밸류에이션: 선행 PER {forward_pe}배, PBR {pbr}배, P/S {ps_ratio}배
                        - 동종 업계(경쟁사) 중앙값: PER {ai_median_pe if 'ai_median_pe' in locals() else 'N/A'}, P/B {median_pb}배

                        [지정된 리포트 양식] (반드시 아래 순서와 텍스트를 100% 똑같이 지켜서 출력할 것. 괄호 안의 지침에 맞춰 분석 내용을 작성할 것. 마침표 뒤에는 무조건 줄바꿈 2번(엔터) 할 것.)
                        
                        ### 1. 비즈니스 모델 및 경제적 해자 : [ A / B / C ] 등급
                        - **분석 요약:** (무엇으로 돈을 버는지, 모닝스타 기준 해자의 넓이는 어떠한지 평가)
                        - **핵심 근거:** (예: 대체 불가능한 전환 비용, 강력한 독점력, 네트워크 효과 등)

                        ### 2. 재무 건전성 및 수익성 (Alpha Spread 기준) : [ A / B / C ] 등급
                        - **분석 요약:** (매출총이익률, 영업이익률, 제공된 ROE 데이터 등을 기반으로 핵심 마진율 평가)
                        - **핵심 근거:** (예: 동종 업계 대비 압도적인 마진율, 무차입에 가까운 건전성 등)

                        ### 3. 경영진 및 주주 거버넌스 : [ A / B / C ] 등급
                        - **분석 요약:** (경영진의 자본 배치 능력, 주주 친화 정책 평가)
                        - **핵심 근거:** (제공된 배당성향 데이터를 참고하여 꾸준한 배당 성장, 자사주 매입 이력 등 서술)

                        ### 4. 밸류에이션 및 안전마진 (Finbox 기준) : [ A / B / C ] 등급
                        - **분석 요약:** (제공된 PER, PBR, PSR 등의 멀티플이 역사적 평균 대비 싼지 비싼지 평가)
                        - **핵심 근거:** (예: 적용 모델 기준 내재 가치 대비 저평가 상태, 혹은 프리미엄 구간 등 팩트 서술)

                        ### 5. 촉매제(Catalyst) 및 리스크 : [ A / B / C ] 등급
                        - **분석 요약:** (향후 주가를 폭발시킬 모멘텀과 치명적인 하락 리스크를 양면 평가)
                        - **핵심 근거:** (예: 신제품 출시, 핵심 수주 공시 기대감 vs 거시 경제 금리 인상 취약 등)

                        ### 6. 동종 업계 멀티플 비교
                        - (제공된 동종 업계 중앙값 PER, PBR 등과 해당 기업의 수치를 직접 비교하여 상대적 고평가/저평가 여부 명확히 결론 낼 것)

                        ### 7. 주요 판매처 및 밸류체인 확인
                        - (이 기업의 핵심 고객사(B2B/B2C) 및 공급망 내 지위 서술)

                        ---
                        ### 🏆 최종 종합 등급 : [ A / B / C ]
                        - **투자 결론:** (5가지 항목과 퀀트 스코어 {score}점을 종합한 최종 투자 매력도 및 추천 코멘트)
                        - **트레이딩 전략:** (최근 수급 동향과 차트 위치를 고려한 1차 진입 타점 및 손절/비중 조절 라인 제안)

                        **💡 평가 기준 가이드**
                        * A등급: 강력 매수 고려 (모든 조건이 충족되고 가격도 매력적)
                        * B등급: 관망 및 조정 시 매수 (기업은 좋으나 밸류에이션이 비싸거나, 모멘텀이 부족함)
                        * C등급: 투자 보류 및 매도 (비즈니스 모델이 훼손되었거나 재무가 위험한 상태)

                        [해시태그 규칙] 맨 마지막에 띄어쓰기 없이 단어 앞에 '#'을 붙여서 쉼표로 나열할 것.
                        예시: #{company_name}주가, #{company_name}전망, #{company_name}실적, #실전매매, #가치투자, #스윙트레이딩, #주식분석, #주도주, #종목추천, #퀀트투자
                        """
